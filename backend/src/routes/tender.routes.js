...
import { tenders } from '../data/store.js';
    ...
    if (req.query.scope === 'all') {
        ...
        return authenticate(req, res, () => {
            if (!req.user || !['ADMIN', 'AUDITOR'].includes(req.user.role))
                return res.status(403).json({ message: 'Not authorised to view all tender records.' });
...
router.post('/', authenticate, authorize('ADMIN'), (req, res) => {
    ...
    if (!title || !department || !description || !closingDate || !reqs.length || !criteriaList.length || total !== 100)
        return res.status(400).json({ message: 'Provide valid tender details, requirements and criteria totalling 100%.' });
    ...
    const tender = { id: `t-${Date.now()}`, reference: `TND-2026-${String(nextNumber).padStart(3, '0')}`, title: String(title).trim(), department: String(department).trim(), description: String(description).trim(), closingDate: String(closingDate), status: 'DRAFT', requirements: reqs, criteria: criteriaList, applications: 0, createdBy: req.user.id };
...
router.post('/:id/publish', authenticate, authorize('ADMIN'), (req, res) => {
    ...
    if (new Date(tender.closingDate).getTime() <= Date.now())
        return res.status(400).json({ message: 'The closing date must be in the future.' });
