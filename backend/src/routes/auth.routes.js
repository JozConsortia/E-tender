...
router.post('/register', (req, res) => {
    ...
    if (!name || !email || !password || !organisation || !director || !Array.isArray(documents) || documents.length !== 3)
    ...
    const user = { id: `u-${Date.now()}`, name: String(name).trim(), email: String(email).trim().toLowerCase(), password: String(password), role: 'APPLICANT', organisation: String(organisation).trim(), directors: [String(director).trim()], verificationStatus: 'PENDING', verificationDocuments: documents.map(String) };
    users.push(user);
