...
export function syncTenderLifecycle() {
    for (const tender of tenders) {
        if (tender.status === 'PUBLISHED' && new Date(tender.closingDate).getTime() <= Date.now()) {
            ...
            for (const application of applications.filter((item) => item.tenderId === tender.id && item.status === 'SUBMITTED'))
...
export function addAudit(actor, action, target) { log(actor, action, target); }
