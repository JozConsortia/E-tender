...
export default function BecEvaluations() {
  ...
  return <>
    <PageHeader title="Bid evaluation" description={`${application.tenderReference} · ${application.companyName}`} action={<div className="form-actions"><Link className="button secondary" to={`/applications/${application.id}/report`}>View full report</Link><button className="button secondary" onClick={() => navigate('/bec')}>Back to register</button></div>} />
