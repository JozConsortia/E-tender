...
export default function FinalApproval() {
  ...
  const decide = async (outcome: 'APPROVE' | 'RETURN' | 'DECLINE') => {
    ...
    if (outcome === 'APPROVE') {
      await decideApplication(application.id, 'SUCCESSFUL', note)
    } else if (outcome === 'RETURN') {
      await decideApplication(application.id, 'SHORTLISTED', note)
    } else {
      await decideApplication(application.id, 'UNSUCCESSFUL', note)
    }
    navigate('/approver')
  }
