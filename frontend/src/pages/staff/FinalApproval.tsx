...
export default function FinalApproval() {
  ...
  const decide = async (outcome: 'APPROVE' | 'RETURN' | 'DECLINE') => {
    ...
    setSaving(false)
    if (!result.ok) { setError(result.message ?? 'The decision could not be recorded.'); return }
    navigate('/approver')
  }

  return <>
    <PageHeader title="Final award decision" description={`${application.tenderReference} · Authorised final approval`} action={<button className="button secondary" onClick={() => navigate('/approver')}>Back to register</button>} />
